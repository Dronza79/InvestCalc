...
def popup_errors_notification(parent, title_error, errors):
    ...
    layout = (
        [[sg.T(title, text_color='DarkRed')] for title in title_error] +
        [[sg.Col([[sg.Text(error)] for error in errors], **col_popup_error)],
         [sg.Button('OK', s=10, button_color='white on DarkRed')]]
    )
